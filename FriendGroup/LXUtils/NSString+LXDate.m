...
#import "NSString+LXDate.h"

@implementation NSString (LXDate)
...
+(NSInteger)int_timeIntervalFromTimeStr:(NSString *)timeStr{
    NSDateFormatter *fmt = [[NSDateFormatter alloc] init];
    // 设置日期格式(为了转换成功)
    fmt.dateFormat = @"yyyy-MM-dd HH:mm:ss";
    // NSString * -> NSDate *
...
+(NSInteger)getIntervalFomeTimeStr1:(NSString *)str1 timeStr2:(NSString *)str2{
    ...
    NSInteger time1 = [self int_timeIntervalFromTimeStr:str1];
    NSInteger time2 = [self int_timeIntervalFromTimeStr:str2];
    
    //    NSLog(@"%ld",time1 - time2);
    return labs(time1 - time2);
